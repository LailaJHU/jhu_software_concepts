...
def original_degree_breakdown(session) -> list[tuple[str, int]]:
    ...
    # Converted to plain tuples (not left as SQLAlchemy Row objects) so the
    # result is safely JSON-serializable -- this list goes straight into
    # jsonify() from the /update-analysis route, not just the HTML template.
    return [(degree, total) for degree, total in session.execute(stmt).all()]
