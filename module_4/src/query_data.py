...
two original questions) using raw SQL executed through ``psycopg`` against
the ``applicants`` table. All numeric formatting follows the assignment's
rules: counts as whole numbers, percentages/averages to 2 decimal places
with '%' where applicable.

``get_connection`` accepts an optional ``dsn`` so callers (including the
test suite) can point it at a different database than the process
...
def build_dsn() -> str:
    ...
    (or ``DATABASE_URL`` if that is set instead, translated to a plain DSN
    psycopg can use directly)."""
    database_url = os.environ.get("DATABASE_URL")
    if database_url:
        # psycopg.connect() accepts a "postgresql://" URL directly too.
...
def get_connection(dsn: str | None = None):
    """Opens a psycopg connection. Pass ``dsn`` explicitly (as the test
    suite does, pointing at a throwaway test database) to bypass the
    environment entirely."""
    return psycopg.connect(dsn or build_dsn())


# ---------------------------------------------------------------------------
# Each question is a SQL string; formatter behavior lives in main().
# ---------------------------------------------------------------------------
...
def run(cur, sql: str):
    """Executes ``sql`` on an already-open cursor and returns every row."""
    cur.execute(sql)
    return cur.fetchall()


def main(dsn: str | None = None):  # pragma: no cover - CLI console output; get_connection()/run() are covered directly
